...
const getAttendanceBySecId = async (req, res) => {
  try {
    ...
    const attendanceData = await prisma.$queryRaw`
      SELECT TO_CHAR(d.created_at,'YYYY-MM-DD') as created_at, ed.student_id, s.firstname, s.midname, s.lastname, COALESCE(ca.status, 'Absent') AS status
      FROM (SELECT DISTINCT created_at::date FROM class_attendance WHERE section_id = 1001) as d
      ...
      WHERE ed.student_id = s.id AND ed.section_id = 1001
